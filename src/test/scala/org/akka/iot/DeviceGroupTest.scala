...
class DeviceGroupTest(_system: ActorSystem)
  ...
  val huhu = system.actorSelection("")
  ...
  "A DeviceGroup Actor" should
    ...
    //Check that device actors are working
    deviceActor1.tell(Device.RecordTemperature(requestId = 0, value = 1.0), probe.ref)
    probe.expectMsg(Device.TemperatureRecorded(requestId = 0))
    deviceActor2.tell(Device.RecordTemperature(requestId = 1, value = 2.0), probe.ref)
    probe.expectMsg(Device.TemperatureRecorded(requestId = 1))
